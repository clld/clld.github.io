...
\abstract{The \em{Cross-Linguistic Linked Data} project (CLLD)\footnote{\url{http://clld.org}} helps record the world's language diversity heritage 
...
on the publication platform; in doing so we hope to make published resources -- i.e.~the interface to the data -- more adaptable.%\footnote{This may be called the
...
Arguably, using a concept of languages as sets of doculects (following \newcite{dlc623}), the \emph{thing} can to some extent be identified 
with the web document describing it anyway.
%    (which is the opposite solution to the one proposed in http://www.ibiblio.org/hhalpin/homepage/publications/indefenseofambiguity.html "In defense of ambiguity")
